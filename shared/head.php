<?php 
    $version = "1.0.0";
    require_once __DIR__ . '/path.php'; // NOTE: Подключение базового пути
?>
...
<head>
    ...
    <meta property="og:image" content="<?= $basePath ?>/files/card_for_messengers.jpg">
    <meta property="og:url" content="<?= htmlspecialchars($ogUrl ?? $basePath . '/') ?>">
    ...
    <meta name="twitter:image" content="<?= $basePath ?>/files/card_for_messengers.jpg">

    <!-- NOTE: Icon for iOS -->
    <link rel="apple-touch-icon" sizes="512x512" href="<?= $basePath ?>/files/apple-touch-icon.png">

    <link rel="icon" href="<?= $basePath ?>/files/favicon.ico">
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@100..900&display=swap" rel="stylesheet">

    <?php if (!isset($noStyle) || !$noStyle): ?>
        <link rel="stylesheet" href="<?= $basePath ?>/public/style.css?v=<?php echo $version; ?>">
    <?php endif; ?>
    ...
    <?php if (isset($loginCss)): ?>
        <link rel="stylesheet" type="text/css" href="<?= $basePath ?>/features/auth/login.css?v=<?= htmlspecialchars($loginCss) ?>&version=<?php echo $version; ?>">
    <?php endif; ?>
