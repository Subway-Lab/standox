<?php
    // NOTE:Включаем вывод ошибок (только для отладки)
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        // NOTE: Получаем данные из формы для таблицы orders
        ...
        // NOTE: Получаем итоговую сумму из скрытого поля формы
        $total_price = $_POST['total_price'];
        $total_price = (int)$total_price;  // Преобразуем к целому числу

        // NOTE: Генерация текущей даты и времени
        $date = date("Y-m-d H:i:s");
        
        // NOTE: Вставка данных в таблицу orders
    ...
    if (isset($_POST['services']) && is_array($_POST['services'])) {
        ...
        // NOTE: Перенаправляем на страницу подтверждения с ID заказа
